...
# from feature_extraction.config import BANWORDS
# import tldextract
...
def get_key_words(string):
    cleanPayload = re.sub(r'\s+', ' ', getpayload(string))
    # print(string)
    wordregex = re.compile(BANWORDS,re.IGNORECASE)
    results = re.findall(wordregex,cleanPayload)
    # print(results)
    return results
